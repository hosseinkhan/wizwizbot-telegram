...
$robotState = $botState['botState']??"on";
 if($userInfo['step'] == "banned" && $from_id != $admin && $userInfo['isAdmin'] != true){
     sendMessage("❌ | هی بهت گفتم گوش نکردی ، الان مسدود شدی 😑😂");
     exit();
 }
if ($joniedState== "kicked" || $joniedState== "left"){
    sendMessage("❌ برای استفاده از ربات حتما باید در کانال زیر عضو باشی، بعد از اینکه عضو شدی برگرد دوباره همینجا 

🆔 $channelLock

😊در این کانال آموزش ها و اطلاع رسانی های گروهی انجام میشه😊

✅ بعد از اینکه عضو شدید مجدد ربات رو /start کن و لذت ببر 
...
if($userInfo['phone'] == null && $from_id != $admin && $userInfo['isAdmin'] != true && $botState['requirePhone'] == "on"){
    if(isset($update->message->contact)){
        ...
        sendMessage("سلام عزیزم، برای شروع شماره تماس خود را با استفاده از کلید 

☎️ ارسال شماره

که در پایین این صفحه میباشد ارسال کن، نیازی نیست به صورت دستی شماره خودت را وارد کنی 😊

برو پایین پایین صفحه اون ته ته 👇", json_encode([
			'keyboard' => [[[
					'text' => '☎️ ارسال شماره',
...
if (preg_match('/^\/([Ss]tart)/', $text) or $text == '⤵️ برگرد به منوی اصلی ' or $text == '🔙بازگشت به منوی اصلی' or $data == 'mainMenu') {
    ...
    if(isset($data) and $data == "mainMenu"){
        $res = editText($message_id, 'سلام سلام 😍

به ربات  فروش فیلتر شکن ما خوش اومدی🌸

ما اینجاییم تا شما را بدون هیچ محدویتی به شبکه جهانی متصل کنیم ❤️

💫 تکنولوژی Warp+ فعاله، پس به راحتی و بدون اضطراب

✅ وارد سایت های بانکی آمریکا و اروپا شو.

✅ از اکانت های خودت در اسپاتیفای و نتفلیکس استفاده کن.

✅ وارد اکانت های ترید، کریپتو کارنسی و غیره شو.

✅ فرم های دولتی و ایالتی اروپا و آمریکا را داخل سایت ها پر کن.

✅ اگه یک موقع سایت خاصی رو دوست داری من به این سیستم اضافه کنیم روی دکمه 📨 درخواست پشتیبانی بزن و درخواست خودت رو ارسال کن.

❓هر موقع ندونستی چیکار کنی برام  بنویس

/start 
', $mainKeys);
        if(!$res->ok){
            sendMessage('سلام سلام 😍
...
if($userInfo['step'] == "addNewAdmin" && $from_id === $admin && $text != $cancelText){
    if(is_numeric($text)){
        ...
        sendMessage("یک خطا رخ داده است دوباره تلاش کن");
    }
}
...
if($userInfo['step'] == "editInviteAmount"){
    if(is_numeric($text)){
        ...
        sendMessage("✅ تنظیمات بازاریابی",$keys);
        setUser();
    }else sendMessage("یک خطا رخ داده است دوباره تلاش کن");
}
...
if ($data=='buySubscription' && ($botState['sellState']=="on" || ($from_id == $admin || $userInfo['isAdmin'] == true))){
    ...
    editText($message_id, '🌸عزیزم🌸

قبل از اینکه شروع کنی برو از قسمت دانلود نرم افزارها اپلیکیشن مناسب خودت رو دانلود کن
...
if($data=="supportSection"){
    ...
    😍درهنگام قطعی های سراسری من برات یک کلید گذاشتم که مطمئن باشم هیچوقت قطعی نداری کلیدت توی منوی اصلیه بزن روی دکمه 

    کلید پشتیبان نرم افزار Outline 
    
    و کلیدت رو بردار ❤️. این کلید رو باید توی نرم افزار های پشتیبان بندازی پس برگرد به صفحه اصلی و نرم افزار پشتیبان مناسب دستگاهت رو از قسمت

    🔗 دانلود نرم افزارها 
    
    بگیر من فدای اون چشمای قشنگت بشم 😍.
    
    اگه موضوع دیگه ای هست لطفا، یکی از دکمه های زیر را انتخاب کن.    
    ",
        json_encode(['inline_keyboard'=>[
        [['text'=>"📮 لیست تیکت ها", 'callback_data'=>"userAllTickets"]],
        [['text'=>"تیکت های باز 📨",'callback_data'=>"usersOpenTickets"]],
        [['text'=>"✉️ ثبت درخواست پشتیبانی",'callback_data'=>"usersNewTicket"]],
        [['text'=>"برگشت 🔙",'callback_data'=>"mainMenu"]]
        ]]));
}
